...
const logBalances = () => {
  return new Promise(async (resolve, reject) => {
    let myWalletEthBalance = +web3.utils.fromWei(
      await web3.eth.getBalance(myWalletAddress)
    );
    let myWalletCEthBalance =
      (await cEth.methods.balanceOf(myWalletAddress).call()) / 1e8;
    let myWalletUnderlyingBalance =
      +(await underlying.methods.balanceOf(myWalletAddress).call()) /
...
const main = async () => {
  ...
  const trx = await cToken.methods
  ...
  let balance = await cToken.methods
    .borrowBalanceCurrent(myWalletAddress)
    .call();
  ...
  await underlying.methods
    .approve(cTokenAddress, underlyingToRepay)
    .send(fromMyWallet);

  const repayBorrow = await cToken.methods
